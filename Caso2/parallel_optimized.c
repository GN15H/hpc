...
void multiplyMatrices(int size, int (*A)[size], int (*B)[size],
                      int (*resultMatrix)[size]);
void printMatrix(int size, int matrix[size][size]);
void fillMatrixWithRandoms(int size, int matrix[size][size]);
void calculateMatrixCell(int size, int row, int col, int A[size][size],
                         int B[size][size], int result[size][size]);
void parallel_multiplyMatrices(unsigned int threadAmount,
                               struct mattrmult_data data[threadAmount],
                               int size, int (*A)[size], int (*B)[size],
                               int (*resultMatrix)[size]);
...
int main(int argc, char **argv) {
  ...
  parallel_multiplyMatrices(threadAmount, data, matrixSize, matrixA, matrixB,
                            result);
...
void parallel_multiplyMatrices(unsigned int threadAmount,
  ...
  for (int i = 0; i < threadAmount; i++) {
    ...
    th_err =
        pthread_create(&(threadList[i]), NULL, mattr_wrapper, (void *)&data[i]);
    if (th_err) {
...
void *mattr_wrapper(void *data) {
  ...
  for (int i = 0; i < args->amount; i++)
    calculateMatrixCell(args->size, (args->pos + i) / args->size,
                        (args->pos + i) % args->size, args->A, args->B,
                        args->result);
  return NULL;
}
